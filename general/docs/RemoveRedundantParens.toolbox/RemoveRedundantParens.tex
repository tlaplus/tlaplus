...
\setboolean{shading}{true}
 ...
 RemoveRedundantParens}\moduleRightDash\@xx{}%
\@x{ {\EXTENDS} Integers ,\, Sequences}%
\par\vspace{8.0pt}%
\@x{ {\CONSTANT} TokId}%
\par\vspace{8.0pt}%
 \@x{ Token \.{\defeq} [ type \.{:} \{\@w{left} ,\,\@w{right} ,\,\@w{other} \}
 ,\, id \.{:} TokId ]}%
\par\vspace{8.0pt}%
\@x{ {\RECURSIVE} ParenDepth ( \_ ,\, \_ )}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
This is a comment in which \ensuremath{t.type \.{>} t.id} so it looks nice.
\end{cpar}%
\end{lcom}%
\@x{ ParenDepth ( seq ,\, i ) \.{\defeq}}%
\@x{\@s{8.2} {\IF} i \.{=} 0}%
\@x{\@s{16.4} \.{\THEN} 0}%
 \@x{\@s{16.4} \.{\ELSE} {\CASE} seq [ i ] . type \.{=}\@w{left}\@s{8.13}
 \.{\rightarrow} ParenDepth ( seq ,\, i \.{-} 1 ) \.{+} 1}%
 \@x{\@s{55.91} {\Box} seq [ i ] . type\@s{10.30} \.{=}\@w{right}\@s{2.05}
 \.{\rightarrow} ParenDepth ( seq ,\, i \.{-} 1 ) \.{-} 1}%
 \@x{\@s{55.91} {\Box} seq [ i ] . type\@s{10.30} \.{=}\@w{other}
 \.{\rightarrow} ParenDepth ( seq ,\, i \.{-} 1 )}%
\par\vspace{8.0pt}%
 \@x{ IsWellFormed ( seq ) \.{\defeq} \.{\land} \A\, i \.{\in} 1 \.{\dotdot}
 Len ( seq ) \.{:} ParenDepth ( seq ,\, i ) \.{\geq} 0}%
\@x{\@s{101.98} \.{\land} ParenDepth ( seq ,\, Len ( seq ) ) \.{=} 0}%
\@x{ ExprOfMaxLen ( n ) \.{\defeq}}%
 \@x{\@s{8.2} {\UNION} \{ \{ s \.{\in} [ 1 \.{\dotdot} i \.{\rightarrow} Token
 ] \.{:} IsWellFormed ( s ) \} \.{:} i \.{\in} 0 \.{\dotdot} n \}}%
\@x{}\midbar\@xx{}%
\par\vspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
The basic idea of the following algorithm is that it walks along the
 expression keeping \ensuremath{unmatchedLeft} equal to the sequence
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{T}{F}{10.0}{0}{}%
 \ensuremath{{\langle} {\langle}i\_1,\, i\_1\.{+}1,\, \.{\dots},\,
 i\_1\.{+}j\_1{\rangle},\, {\langle}i\_2,\, i\_2\.{+}2,\, \.{\dots},\,
 i\_2\.{+}j\_2{\rangle} ,\, \.{\dots} {\rangle}
}%
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{1}{F}{F}{0}{0}{}%
 where the element \ensuremath{{\langle}i\_k,\, \.{\dots},\,
 i\_k\.{+}j\_k{\rangle}} means that there is a sequence
 of consecutive left parens at position \ensuremath{i\_k}, \.{\dots} ,
 \ensuremath{i\_k \.{+} j\_k} for which
 the corresponding right parens have not been encountered. Left parens
 and \ensuremath{\@w{other}} tokens are put into \ensuremath{out} as they are
 found. Left parens
 are removed from \ensuremath{out} when their matching right parens are found
 and
 the pair are found to be redundant. A right parens is also put into
 \ensuremath{out} immediately and removed when it is determined to be
 redundant,
 which will be on the next iteration. Note that left parens are removed
 from out from right to left, so the index of the left \ensuremath{paren}
 that is to
 be removed has not been changed because of the previous removal of a
 left \ensuremath{paren}.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
--algorithm Remove \ensuremath{\{
}%
\end{cpar}%
\begin{cpar}{0}{T}{F}{5.0}{0}{}%
\ensuremath{variables} \ensuremath{in \.{\in} ExprOfMaxLen(5),\,
}%
\end{cpar}%
\begin{cpar}{0}{T}{F}{25.0}{0}{}%
\ensuremath{out \.{=} {\langle} {\rangle},\,
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
\ensuremath{unmatchedLeft \.{=} {\langle} {\rangle},\,
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
\ensuremath{i \.{=} 1,\,
...
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
\ensuremath{justFoundLeft \.{=} {\FALSE},\,
}%
\end{cpar}%
\begin{cpar}{0}{T}{F}{5.0}{0}{}%
 \ensuremath{\.{\,\backslash\,}\.{*} true} \ensuremath{means}
 \ensuremath{that} \ensuremath{the} \ensuremath{token} \ensuremath{at}
 \ensuremath{i\.{-}1} \ensuremath{is} \ensuremath{a} \ensuremath{left}
 \ensuremath{paren
}%
\end{cpar}%
\begin{cpar}{1}{F}{F}{0}{0}{}%
\ensuremath{justFoundRight \.{=} {\FALSE};
}%
\end{cpar}%
\begin{cpar}{0}{T}{F}{5.0}{0}{}%
 \ensuremath{\.{\,\backslash\,}\.{*} true} \ensuremath{means}
 \ensuremath{that} \ensuremath{the} \ensuremath{token} \ensuremath{at}
 \ensuremath{i\.{-}1} \ensuremath{is} \ensuremath{a} \ensuremath{right}
 \ensuremath{paren
}%
\end{cpar}%
\begin{cpar}{3}{T}{F}{2.5}{0}{}%
\ensuremath{\{ while (i \.{\leq} Len(in)) \{
}%
\end{cpar}%
\begin{cpar}{0}{T}{F}{12.5}{0}{}%
\ensuremath{if (in[i].type \.{=} \@w{left}) \{
}%
\end{cpar}%
\begin{cpar}{0}{T}{F}{5.0}{0}{}%
\ensuremath{if (justFoundLeft) \{
}%
\end{cpar}%
\begin{cpar}{0}{T}{F}{5.0}{0}{}%
\ensuremath{unmatchedLeft[Len(unmatchedLeft)] \.{:=}
}%
\end{cpar}%
\begin{cpar}{0}{T}{F}{5.0}{0}{}%
\ensuremath{Append(unmatchedLeft[Len(unmatchedLeft)],\, i);
}%
\end{cpar}%
\begin{cpar}{1}{F}{F}{0}{0}{}%
\ensuremath{out \.{:=} Append(out,\, in[i])
}%
\end{cpar}%
\begin{cpar}{1}{F}{F}{0}{0}{}%
\ensuremath{\}
...
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
\ensuremath{else} \ensuremath{if (in[i].type \.{=} \@w{right}) \{
}%
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 \ensuremath{Dec} 19 18:17:21 \ensuremath{PST} 2011 by \ensuremath{lamport
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Mon} \ensuremath{Dec} 19
 17:20:10 \ensuremath{PST} 2011 by \ensuremath{lamport
}%
\end{cpar}%
\end{lcom}%
\end{document}
