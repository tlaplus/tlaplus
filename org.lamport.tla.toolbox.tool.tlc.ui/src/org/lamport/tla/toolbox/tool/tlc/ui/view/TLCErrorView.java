...
public class TLCErrorView extends ViewPart {
	...
	protected void fill(String modelName, List problems) {
		...
		if (problems != null && !problems.isEmpty()) {
			...
			boolean isNewTrace = states != null && oldStates != null
					&& !(states == oldStates);
	...
	public void createPartControl(Composite parent) {
		...
		errorViewer = FormHelper.createFormsOutputViewer(toolkit,
				outerSashForm, SWT.V_SCROLL | SWT.H_SCROLL | SWT.MULTI
						| SWT.BORDER);
		gd = new GridData(SWT.FILL, SWT.FILL, true, false);
		gd.heightHint = 100;
	...
	static class TraceDisplayResizer extends ControlAdapter implements Listener {
		...
		public void controlResized(ControlEvent e) {
			...
			int firstColWidth = Math.round(Math
					.round(firstColumnPercentageWidth * treeWidth));
		...
		public void handleEvent(Event event) {
			...
			double tentativefirstColPercentageWidth = (double) firstColWidth
					/ (double) treeWidth;
			if (tentativefirstColPercentageWidth > .1
					&& tentativefirstColPercentageWidth < .9) {
				firstColumnPercentageWidth = (double) firstColWidth
						/ (double) treeWidth;
			...
			firstColWidth = Math.round(Math
					.round(tentativefirstColPercentageWidth * treeWidth));
			int secondColWidth = treeWidth - firstColWidth;
			column[1].setWidth(secondColWidth);
		}
		...
		private int computeMaximumWidthOfAllColumns() {
			...
			return comp.getClientArea().width - tree.getBorderWidth()
...
//			setImage = TLCUIActivator.getImageDescriptor("SPbaun1.gif").createImage();
		}
		...
		public Image getColumnImage(Object element, int columnIndex) {
			if (columnIndex == NAME) {
				...
				return varImage; // setImage; // other things appear in unordered collections
			}
			return null;
		}
