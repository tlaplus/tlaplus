...
/**
 * Test suite requirements: - vncserver on localhost:1 -- on Linux you have to
 * start a window manager explicitly like fluxbox (.vnc/xstartup) -
 * osgi.framework.extensions set to org.eclipse.equinox.weaving.hook - Bundle
 * org.eclipse.equinox.weaving.aspectj explicitly started on runlevel (default -
 * 1) => e.g. 3
 */
...
public class HandlerThreadingTest extends AbstractTest {
	...
	public static void beforeClass() {
		...
		Assert.assertTrue("Given spec file does not exist: " + specB, new File(specB).exists());
	...
	/**
	 * Checks how many invocations of backend code have happend inside the UI
	 * thread
	 */
	private void assertNoBackendCodeInUIThread() {
		...
		Assert.assertFalse("Backend code (e.g. parsing must not be executed in UI thread) times executed: "
				+ MonitorAdaptor.getTriggeredJoinPoints().size(), MonitorAdaptor.hasTriggeredBackendCode());
