...
THEOREM BoundaryIntervalFcnSetPermutation ==
  Cases!BoundaryIntervalFcnSetPermutation
  BY DEF Cases!BoundaryIntervalFcnSetPermutation,
         Cases!BoundaryIntervalFcns

THEOREM BoundaryMixedFcnSetPermutation ==
  Cases!BoundaryMixedFcnSetPermutation
  BY DEF Cases!BoundaryMixedFcnSetPermutation, Cases!BoundaryMixedFcns

THEOREM BoundaryFcnDomainNormalize == Cases!BoundaryFcnDomainNormalize
  BY DEF Cases!BoundaryFcnDomainNormalize,
         Cases!BoundaryFcnDomainNormalizeWith

LEMMA SingletonIntervalIdentityApply ==
  ASSUME NEW n \in Int
  PROVE  [x \in n..n |-> x][n] = n
  OBVIOUS

THEOREM BoundaryFcnSelect == Cases!BoundaryFcnSelect
  <1>1. Cases!MinInt \in Int /\ Cases!MaxInt \in Int
    BY DEF Cases!MinInt, Cases!MaxInt
  <1>2. [x \in Cases!MinInt..Cases!MinInt |-> x][Cases!MinInt]
           = Cases!MinInt
    BY <1>1, SingletonIntervalIdentityApply
  <1>3. [x \in Cases!MaxInt..Cases!MaxInt |-> x][Cases!MaxInt]
           = Cases!MaxInt
    BY <1>1, SingletonIntervalIdentityApply
  <1>4. QED BY <1>2, <1>3
    DEF Cases!BoundaryFcnSelect, Cases!BoundaryFcnSelectOne

LEMMA SingletonIntervalExcept ==
  ASSUME NEW n \in Int
  PROVE  [[x \in n..n |-> 0] EXCEPT ![n] = 1][n] = 1
  OBVIOUS

THEOREM BoundaryFcnExcept == Cases!BoundaryFcnExcept
  <1>1. Cases!MinInt \in Int /\ Cases!MaxInt \in Int
    BY DEF Cases!MinInt, Cases!MaxInt
  <1>2. [[x \in Cases!MinInt..Cases!MinInt |-> 0]
             EXCEPT ![Cases!MinInt] = 1][Cases!MinInt] = 1
    BY <1>1, SingletonIntervalExcept
  <1>3. [[x \in Cases!MaxInt..Cases!MaxInt |-> 0]
             EXCEPT ![Cases!MaxInt] = 1][Cases!MaxInt] = 1
    BY <1>1, SingletonIntervalExcept
  <1>4. QED BY <1>2, <1>3
    DEF Cases!BoundaryFcnExcept, Cases!BoundaryFcnExceptOne

LEMMA CombineDisjointPoint ==
  ASSUME NEW S, NEW n, NEW m, NEW a, NEW b,
         n \notin S, m \in S
  PROVE
    LET g == [x \in (DOMAIN [y \in S |-> a])
                       \cup (DOMAIN [y \in {n} |-> b]) |->
                 IF x \in DOMAIN [y \in S |-> a]
                   THEN [y \in S |-> a][x]
                   ELSE [y \in {n} |-> b][x]]
    IN  DOMAIN g = S \cup {n} /\ g[n] = b /\ g[m] = a
  OBVIOUS

THEOREM BoundaryFcnBinarySelect == Cases!BoundaryFcnBinarySelect
  <1>1. Cases!MaxInt \notin 1..32 /\ 16 \in 1..32
    BY DEF Cases!MaxInt
  <1>2. QED BY <1>1, CombineDisjointPoint
    DEF Cases!BoundaryFcnBinarySelect,
        Cases!BoundaryFcnBinarySelectWith

LEMMA SingletonIntervalFcnEq ==
  ASSUME NEW n \in Int
  PROVE  [x \in n..n |-> x] = [x \in {n} |-> x]
  OBVIOUS

THEOREM FPBoundaryFcns == Cases!FPBoundaryFcns
  <1>1. Cases!MinInt \in Int /\ Cases!MaxInt \in Int
    BY DEF Cases!MinInt, Cases!MaxInt
  <1>2. [x \in Cases!MinInt..Cases!MinInt |-> x]
           = [x \in {Cases!MinInt} |-> x]
    BY <1>1, SingletonIntervalFcnEq
  <1>3. [x \in Cases!MaxInt..Cases!MaxInt |-> x]
           = [x \in {Cases!MaxInt} |-> x]
    BY <1>1, SingletonIntervalFcnEq
