...
public final class LaTeXOutput {
    ...
    public void WriteLaTeXFile(final Token[][] spec) { // BEGIN  WriteLaTeXFile(Token[][] spec)
        final OutputFileWriter writer =
...
// Debug.print2DArray(spec, "spec") ;
        ...
        while (line < spec.length) { // BEGIN while (line < spec.length)
            ...
            if (spec[line].length == 0) {                   //  BEGIN then OF if (spec[line].length == 0)
                /********************************************************************
            ...
            else {                   //  BEGIN else OF if (spec[line].length == 0)
                ...
                if ((spec[line][0].type == Token.COMMENT)
                        ...
                        == CommentToken.MULTI)
                        || (((CommentToken) spec[line][0]).subtype
                        == CommentToken.NULL))) { // BEGIN if ( (spec[line][0].type == Token.COMMENT) ... )
                    /***************************************************************
                    ...
                    if (issueVSpace) { // BEGIN if (issueVSpace)
                        /*****************************************************************
                ...
                if (!issueVSpace) { // BEGIN if (! issueVSpace)
                     ...
                     ********************************************************************/
                    while ((!done)
                            && (item < spec[line].length)) { // BEGIN while (   (! done) && (item < spec[line].length)  )
                        /*******************************************************************
                        ...
                        switch (tok.type) { // BEGIN switch (tok.type)
                            case Token.BUILTIN:
                                final int symType = Objects.requireNonNull(BuiltInSymbols.GetBuiltInSymbol(
                                        spec[line][item].string, true)).symbolType;
                                if ((!inSub)
