...
public class TLAplusParser implements tla2sany.st.SyntaxTreeConstants, ParseTree, TLAplusParserConstants {
  ...
  /**
   * A {@link PrintStream} instance to which parser trace output is logged.
   */
  private PrintStream traceOut = new PrintStream(OutputStream.nullOutputStream());

  /**
   * Subscribe the given {@link PrintStream} instance to the parser's trace
   * output. This logs every time the recursive descent parser enters & exits
   * a parse rule; it is very verbose and should only ever be necessary when
   * conducting development work on the parser itself. By default, all trace
   * output is sent to {@link OutputStream#nullOutputStream()}.
   *
   * @param traceOut A {@link PrintStream} instance to receive trace output.
   */
  public void routeTraceOutputTo(PrintStream traceOut) {
...
//
  ...
  *************************************************************************/
  ...
  private final void bpa( String s) { // Beginning of Production Actions 
    addHeir( null );
    this.traceOut.println("Beginning " + s);
  ...
  private final void epa() {
    popMsg();
    this.traceOut.println("Ending " + msgStack[msgStackCurrentSize]);
