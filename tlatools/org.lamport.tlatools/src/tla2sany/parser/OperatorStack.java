...
public class OperatorStack implements tla2sany.st.SyntaxTreeConstants {
  ...
  private final ArrayList<ArrayList<OSelement>> StackOfStack = new ArrayList<>(10);
    ...
    * a vector v where v.get(v.size()-1) is the top element of the   *
    * stack.                                                               *
    ***********************************************************************/
...
// could be optimized to reuse memory
  public final void newStack() {
    ...
    ***********************************************************************/
    CurrentTop = new ArrayList<OSelement>( 20 );
  ...
  final public boolean preInEmptyTop() {
    ...
    ***********************************************************************/
    ...
    if (CurrentTop.size() == 0)
      ...
      Operator op  = CurrentTop.get(CurrentTop.size() - 1).getOperator();
      if (op != null)
...
//    SyntaxTreeNode localTN = new InfixExprNode();
    if (n>=3) {
      SyntaxTreeNode opNode  = CurrentTop.get(n-2).getNode();
      SyntaxTreeNode leftOp  = CurrentTop.get(n-3).getNode();
      SyntaxTreeNode rightOp = CurrentTop.get(n-1).getNode();
...
//      localTN.addChild( (CurrentTop.get( n-2)).getNode() ) ;
//      localTN.addChild( (CurrentTop.get( n-1)).getNode() ) ;
      CurrentTop.remove(n-1);
...
//    SyntaxTreeNode localTN = new PostfixExprNode();
    if (n>=2) {
      Operator op = CurrentTop.get(n-1).getOperator();
      SyntaxTreeNode opNode = CurrentTop.get(n-1).getNode();
      if (op != fcnOp ) {
//      ((GenOpNode)opNode).register( op.getIdentifier(), STable);
...
// System.out.println("postfix reduction : FcnOp");
        SyntaxTreeNode eSTN = CurrentTop.get(n-2).getNode();
...
// System.out.println("exit at size: " + (CurrentTop.size()-1));
  ...
  final public SyntaxTreeNode finalReduce() throws ParseException {
    ...
    if ( isWellReduced() )
      ...
      int l[];
      do {
//(CurrentTop.get(n)).getNode().printTree(new java.io.PrintWriter(System.out));  n++;
        msg.append("-- incomplete expression at ");
        msg.append(CurrentTop.get(n).getNode().getLocation().toString() ) ;
        msg.append(".\n");
        n++;
      } while (n < CurrentTop.size()-1);
...
//    System.out.println(printStack());
  ...
  public final void reduceRecord( SyntaxTreeNode middle, SyntaxTreeNode right ) throws ParseException {
    ...
    if ( oselt.isOperator() ) {
      OSelement ospelt = CurrentTop.get( index - 1 );
    ...
    SyntaxTreeNode left = CurrentTop.get(index).getNode();
  ...
  final private String printStack() {
    String str = new String( "stack dump, " + StackOfStack.size() + " levels, " + CurrentTop.size() + " in top one: ");
     for (int i = 0; i< CurrentTop.size(); i++ ) {
       SyntaxTreeNode tn = CurrentTop.get(i).getNode();
       if (tn != null)
