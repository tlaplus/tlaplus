/****************************************************************************
 * CLASS PcalTLAGen                                                         * 
 * last modified on Thu  6 March 2008 at 10:16:22 PST by lamport               *
 *      modified on Mon  3 Oct 2005 at 21:43:09 UT by keith                 *
 *                                                                          *
 * Given an exploded and disambiguated AST, generate the equivalent TLA+.   *
 * There is one method:                                                     *
 *                                                                          *
 *    Vector Gen(AST ast, PcalSymTab st);                                   *
 *                                                                          *
 * This returns a vector of Strings, one entry per line of generated TLA+.  *
 *                                                                          *
 ****************************************************************************/
...
public class PcalTLAGen {
    ...
    private static boolean EmptyExpr(TLAExpr expr) {
        ...
        if (expr.tokens == null || expr.tokens.size() == 0 ) return true;
        return false;
    }
    ...
    private void GenProcedure (AST.Procedure ast, String context) throws PcalTLAGenException {
        ...
        for (int i = 0; i < ast.body.size(); i++) {
            ...
            if ((ns.length() + stmt.label.length()
                 + " \\/ ".length()
                 + ((mp) ? "(self)".length() : 0))
                > wrapColumn - nsC - " \\/ ".length()) {
                nsV.addElement(ns.toString());
                ns = new StringBuffer();
            }
            if (ns.length() > 0) ns.append(" \\/ ");
            ns.append(stmt.label);
            if (mp) ns.append("(self)");
    ...
    private void GenLabeledStmt(AST.LabeledStmt ast, String context) throws PcalTLAGenException {
        ...
        if (mp && (context.equals("procedure")
                   || self.equals("self"))) 
            sb.append("(self)");
        sb.append(" == ");
        int col = sb.length();
        if (ast.stmts.size() > 1) sb.append("/\\ ");
...
***************************************************************************/
    ...
    private void GenAssign(AST.Assign ast,
        ...
        while (i < ast.ass.size()) {
            ...
            if (cThis.Set(sF.lhs.var) > 1
                || (iLast - iFirst > 0 && EmptyExpr(sF.lhs.sub)))
                /***********************************************************
            ...
            if (iFirst == iLast) {
                ...
                TLAExpr rhs = AddSubscriptsToExpr(sass.rhs,
                                                  SubExpr(Self(context)),
                                                  c);
                if (mp && (sass.lhs.var.equals("pc") ||
                           IsProcedureVar(sass.lhs.var) ||
                           IsProcessSetVar(sass.lhs.var) ||
                ...
                sb.append(" EXCEPT ");
                int cc = sb.length();
                boolean subscript = (mp && (IsProcedureVar(sass.lhs.var)
                                            || IsProcessSetVar(sass.lhs.var)));
                while (iFirst <= iLast) {
                    ...
                    sb.append("!");
                    int here = sb.length();
                    if (subscript) sb.append("[" + Self(context) + "]");
                    Vector sv = sub.toStringVector();
                    if (sv.size() > 0) {
                        sb.append((String) sv.elementAt(0));
                        for (int v = 1; v < sv.size(); v++) {
        ...
        if (vlines.size() > 0) {
            for (int v1 = 0; v1 < vlines.size(); v1++) {
                ...
                for (int  v2 = 0; v2 < vl.size(); v2++) {
                    ...
                    tlacode.addElement(sb.toString());
                    sb = new StringBuffer(NSpaces(col));
                    if ((v1 > 0 || numAssigns > 1)
                        && (v2 != vl.size() - 1)) sb.append("   ");
                }
                sb.append("/\\ ");
            }
        }
    ...
    private void GenIf(AST.If ast,
        ...
        for (int i = 0; i < ast.Then.size(); i++) {
            GenStmt((AST) ast.Then.elementAt(i),
                    cThen,
                    context,
                    sb.toString(),
        /*******************************************************************
        ...
        *******************************************************************/
        ...
        if (ast.Else.size() == 0) {
            ...
            for (int i = 0; i < ast.Else.size(); i++) {
                GenStmt((AST) ast.Else.elementAt(i),
                        cElse,
                        context,
                        sb.toString(),
        /*******************************************************************
        ...
        *******************************************************************/
        ...
        if (cElse.NumUnchanged(cThen) > 1) {
            Vector uncElse =
                cElse.Unchanged(cThen,
                                wrapColumn -  sb.length() - "UNCHANGED << ".length());
            sb.append("UNCHANGED << ");
            int cc = sb.length();
    ...
    ***********************************************************************/
      ...
      *********************************************************************/
      for (int i = 0; i < ast.ors.size(); i++) {
        ...
        for (int j = 0; j < orClause.size(); j++) {
            GenStmt((AST) orClause.elementAt(j),
                    cC,
                    context,
                    sb.toString(),
                    here);
     ...
     **********************************************************************/
     ...
     while (i > 0)
       { i = i - 1;
         ...
         if ( numUnchanged > 1) {
           tlacode.insertElementAt(
              NSpaces(here) + "/\\ UNCHANGED <<" + NotChanged + ">>",
              ucLocs[i]) ;            
    ...
    private void GenWith(AST.With ast,
        ...
        for (int i = 0; i < ast.Do.size(); i++) {
            GenStmt((AST) ast.Do.elementAt(i),
                    c,
                    context,
                    sb.toString(), 
                    sb.length());
...
// ast.line and ast.col
            ...
            for (int  i = 0; i < procs.size(); i++) {
                ...
                if (proc.params != null)
                    for (int p = 0; p < proc.params.size(); p++) {
                        AST.PVarDecl decl =
                            (AST.PVarDecl) proc.params.elementAt(p);
                ...
                if (proc.decls != null)
                    for (int p = 0; p < proc.decls.size(); p++) {
                        AST.PVarDecl decl = 
                            (AST.PVarDecl) proc.decls.elementAt(p);
                        lVars.addElement(decl.var);
                        vars.addElement(decl.var);
                        pcV.addElement(decl.var);
                    }
        ...
        if (processes != null)
            for (int  i = 0; i < processes.size(); i++) {
                ...
                if (proc.decls != null)
                    for (int p = 0; p < proc.decls.size(); p++) {
                        AST.VarDecl decl =
                            (AST.VarDecl) proc.decls.elementAt(p);
                        lVars.addElement(decl.var);
                        vars.addElement(decl.var);
                        if (! proc.isEq) psV.addElement(decl.var);
                    }
            }
    ...
    ***********************************************************************/
    public void GenVarDecl(Vector varVec) {
        ...
        if (varVec.size() == 0) { return; };
        if (varVec.size() > 1) { res.append("VARIABLES "); } 
        else { res.append("VARIABLE "); } ;
        for (int i = 0; i < varVec.size(); i++) {
          ...
          if (curLine.length() + vbl.length() + 1 > wrapColumn) {
            ...
            else { res.append(NSpaces("VARIABLE ".length())); };
          } ;
          res.append(vbl) ;
          curLine.append(vbl) ;
         } ;
    ...
    public void GenProcSet() {
        ...
        for (int i = 0; i < st.processes.size(); i++) {
            PcalSymTab.ProcessEntry proc =
                (PcalSymTab.ProcessEntry) st.processes.elementAt(i);
            Vector sv = proc.id.toStringVector();
            if (i > 0) ps.append(" \\cup ");
            if (proc.isEq) ps.append("{");
            else ps.append("(");
            int col = ps.length();
    ...
    private void GenInit (Vector globals,
        ...
        if (globals != null && globals.size() > 0) {
            ...
            for (int i = 0; i < globals.size(); i++) {
                AST.VarDecl decl =
                    (AST.VarDecl) globals.elementAt(i);
                is.append("/\\ ");
                is.append(decl.var);
                if (decl.isEq) is.append(" = ");
                else is.append(" \\in ");
                int col2 = is.length();
                    ...
                    *******************************************************/
                        ...
                        if (decl.isEq) {
                            ...
                            ***********************************************/
                            ...
                            expr.addToken(new TLAToken("[", 0, 
                                           TLAToken.BUILTIN));
                            expr.addToken(new TLAToken("CHOOSE", 1, 
                                           TLAToken.BUILTIN));
                            expr.addToken(new TLAToken("self", 8, 
                                                       TLAToken.IDENT));
                    ...
                    if ( ! proc.isEq ) 
                      { expr.addToken(new TLAToken("[", 0, TLAToken.BUILTIN));
                        if (decl.isEq) 
                          { expr.addToken(new TLAToken("self", 1, 
                                                       TLAToken.IDENT));
                            expr.addToken(new TLAToken("\\in ", 6, 
                                                       TLAToken.BUILTIN));
                          } ;
                        expr.normalize() ;
        ...
        if (mp) {
            ...
            int colPC = is.length();
            if (boxUnderCASE) colPC = colPC - 3;
            for (int p = 0; p < st.processes.size(); p++) {
                PcalSymTab.ProcessEntry pe = 
                    (PcalSymTab.ProcessEntry) st.processes.elementAt(p);
                is.append("self ");
                if (pe.isEq) {
                    is.append("= ");
                ...
                if (p == st.processes.size() - 1)
                    is.append("\"]");
                else if (! boxUnderCASE) is.append("\" []");
                else is.append("\"");
                tlacode.addElement(is.toString());
        ...
        if (mp && st.procs.size() > 0) {
            for (int i = 0; i < st.procs.size();  i++) {
                PcalSymTab.ProcedureEntry p
                    = (PcalSymTab.ProcedureEntry) st.procs.elementAt(i);
                if ((p.name.length()
                     + "(self) \\/ ".length()
                     + sb.length())
                    > max) {
            ...
            if (sb.length() > 0) nextSS.addElement(sb.toString() + ")");
        }
    ...
    private void GenSpec () {
        ...
        if (PcalParams.FairnessOption.equals("wfNext") ||
            ...
            String fairness = ((PcalParams.FairnessOption.equals("wf"))
                               ? "WF" : "SF");
        ...
        else tlacode.addElement(sb.toString());
        tlacode.addElement("");
    }
    ...
    private TLAExpr AddSubscriptsToExpr(TLAExpr exprn,
        ...
        for (int i = 0; i < expr.tokens.size(); i++) {
            ...
            for (int j = 0; j < tv.size(); j++) {
                ...
                boolean prime = ((tok.type == TLAToken.IDENT)
                                 && c.IsChanged(tok.string));
                boolean subr = (sub != null
                               && (tok.type == TLAToken.ADDED
                                  || (mp && (tok.type == TLAToken.IDENT) 
                                    && (IsProcedureVar(tok.string)
                ...
                if ((subr || prime) && ! InVector(tok.string, stringVec)) {
                    ...
                    if (prime)
                      ...
                      *****************************************************/
                      exp.addTokenOffset(new TLAToken("'", 
                                                      0,
                                                      TLAToken.BUILTIN),
                                         0);
                    if (subr) {
...
// What the ... is going on here?
    ...
    ***********************************************************************/
    private static Vector Parenthesize(Vector vec) {
      ...
      while (   (curLineNum < vec.size()) 
             && (! needParen)) {
        String curLine  = (String) vec.elementAt(0) ;
        while (   (curCharNum < curLine.length())
               && (! needParen)) {
            ...
            ***************************************************************/
            ...
            switch (curChar) {
              ...
              case '#':
                ...
                break ;
              case '<':
                if (nextChar == '<') {curCharNum++; 
                                     leftParen = true ;}
                else { mayNeedParen = true ;} ;
                break ;
              case '>':
                if (nextChar == '>') {curCharNum++; 
                                     rightParen = true ;}
             ...
             if (mayNeedParen && (parenDepth == 0)) { needParen = true ; } ;
             if (leftParen)  { parenDepth++; } ;
             if (rightParen) {
               if (parenDepth == 0) { needParen = true ; } ;
               parenDepth -- ;
               }
            }; // end else ! inString
          curCharNum++ ;
      ...
      *********************************************************************/
      if (needParen) {
        ...
        vec.setElementAt(((String) vec.elementAt(curLineNum)) + ")",
                        curLineNum) ;
       } ;
      return vec; 
     } 
