...
public class Simulator implements Cancelable {
	...
	class SimulationWorker extends Thread {
		...
		public final void run() {
			...
			while (this.running.get()) {
				try {
					...
					workerFinishedErrChannel.offer(new SimulationWorkerResult(0, null, this.curState, this.stateTrace, e));
					return;
				}
			}
		}
		...
		private Boolean simulateRandomTrace() throws Exception {
			this.curState = null;
			this.stateTrace = new StateVec((int) traceDepth);
			stateTrace.clear();
	...
	public void simulate() throws Exception {
		...
		try {
			...
			if(errInfo.exception != null ) {
				if(errInfo.exception instanceof LiveException) {
					this.printSummary();
				}else {
					printBehavior(EC.GENERAL, new String[] { MP.ECGeneralMsg("", errInfo.exception) }, errInfo.state, errInfo.stateTrace);
				}
			}
			...
			for (SimulationWorker worker : workers) {
				...
				// TODO: Can remove this catch block? (WS 2018)
