...
public class Simulator implements Cancelable {
	...
	private LongAdder numOfGenStates = new LongAdder();
	private LongAdder numOfGenTraces = new LongAdder();
	 ...
	 */
	private void shutdownAndJoinWorkers(ArrayList<SimulationWorker> workers) throws InterruptedException {
	...
	/**
	 * Create a liveness checker object for use by a simulation worker.
	 */
	private ILiveCheck makeLiveCheck(Tool tool) throws IOException {
		if (this.checkLiveness) {
			if (EXPERIMENTAL_LIVENESS_SIMULATION) {
				final String tmpDir = System.getProperty("java.io.tmpdir");
				return new LiveCheck(tool, new Action[0], tmpDir, new DummyBucketStatistics());
			} else {
				return new LiveCheck1(tool);
			}
		} else {
			return new NoOpLiveCheck(tool, specDir);
		}
	}
	...
	public void simulate() throws Exception {
		...
		Set<Integer> runningWorkers = new HashSet<>();
		for (int i = 0; i < this.numWorkers; i++) {
			
			// Give each worker its own Tool and liveness checker object.
			Tool newTool = new Tool(specDir, specFile, configFile, resolver);
			newTool.init(preprocess, null);
			ILiveCheck liveChecker = makeLiveCheck(newTool);
			SimulationWorker worker = new SimulationWorker(i, newTool, initStates, this.workerResultQueue,
					this.rng.nextLong(), this.traceDepth, this.traceNum, this.checkDeadlock, this.traceFile,
					liveChecker, this.numOfGenStates, this.numOfGenTraces, this.astCounts);			
		...
		while (true) {
			SimulationWorkerResult result = workerResultQueue.take();
