...
public class SimulationWorker extends Thread {
	...
	// A counter that tracks the number of generated states. This counter may be
	// shared among workers, so its count may be greater than the number of states
	// generated by this worker alone. It is the duty of this worker, though, to
	// update it whenever it generates a new state.
	...
	public SimulationWorker(int id, Tool tool, StateVec initStates, BlockingQueue<SimulationWorkerResult> resultQueue,
			...
			ILiveCheck liveCheck, LongAdder numOfGenStates, ObjLongTable astCounts) {
		this.id = id;
	...
	public final void run() {
		while(true) {
			try {
				...
				// checks.
				Optional<SimulationWorkerError> res = simulateRandomTrace();
				traceCnt++;
	...
	public final StateVec randomNextStates(RandomGenerator rng, TLCState state) {
		...
		int p = rng.nextPrime();
		for (int i = 0; i < len; i++) {
			StateVec pstates = this.tool.getNextStates(actions[index], state);
			if (!pstates.empty()) {
				return pstates;
			}
			index = (index + p) % len;
	...
	private Optional<SimulationWorkerError> simulateRandomTrace() throws Exception {
		...
		for (int traceIdx = 0; traceIdx < maxTraceDepth; traceIdx++) {
			...
			TLCState s1 = randomState(localRng, nextStates);
