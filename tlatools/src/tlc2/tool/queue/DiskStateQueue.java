...
public class DiskStateQueue extends StateQueue {
  ...
  /* Invariants:
     I1. Entries in deqBuf are in the indices:
         [deqIndex, deqBuffer.length )
     I2. Entries in enqBuf are in the indices:
         [ 0, enqIndex )
     I3. 0 <= deqIndex <= deqBuf.length, where
         deqIndex == 0 => buffer full
         deqIndex == deqBuf.length => buffer empty
     I4. 0 <= enqIndex <= enqBuf.length, where
         enqIndex == 0 => buffer empty
         enqIndex == enqBuf.length => buffer full
  */
  ...
  private final void fillDeqBuffer() {
	...
	if (buf != null) {
	  ...
	  System.arraycopy(this.enqBuf, 0,
			   this.deqBuf, this.deqIndex,
			   this.enqIndex);
	  this.enqIndex = 0;
	}
      }
