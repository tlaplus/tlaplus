...
implements Cloneable, Serializable {
  ...
  static public final long serialVersionUID = 2810641868555561165L;
  
  
  private Value[] values;
  // if it becomes a performance issue to use this collection, switch to another which preserves instance singularity
  private HashSet<SemanticNode>[] asts;
  ...
  private TLCStateMutSource(Value[] vals, HashSet<SemanticNode>[] asts) {
    this.values = vals;
    this.asts = asts;
  }
  ...
  public static void init(Tool tool) {
    ...
    HashSet<SemanticNode>[] snodes = new HashSet[vars.length];
  ...
  public synchronized TLCState registerCoverage(UniqueString name, SemanticNode ast) {
	if (this.asts != null) {
	  ...
	  HashSet<SemanticNode> hs = this.asts[index];
  ...
  public final TLCState unbind(UniqueString name) {
    int loc = name.getVarLoc();
    HashSet<SemanticNode> hs = this.asts[loc];
  ...
  public final TLCState copy() {
    ...
    HashSet<SemanticNode>[] astClone = null;
  ...
  public final void addCounts(ObjLongTable counts) {
	if (this.asts != null) {
	  for (int i = 0; i < this.asts.length; i++) {
	    	HashSet<SemanticNode> hs = this.asts[i];
