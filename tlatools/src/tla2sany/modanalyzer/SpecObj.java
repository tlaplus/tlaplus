...
public class SpecObj {
  ...
  public NameToFileIStream ntfis = new NameToFileIStream();
    /***********************************************************************
  ...
  public SpecObj(String pfn) { primaryFileName = pfn; }
   ...
   */
  public final int          getErrorLevel()      { return errorLevel; }
  ...
  public final void         setName(String name) { rootModuleName = name; }

  /**
   * Returns the raw file name of the top-level module as passed to
   * the method Specification.frontEndMain().
   */
  public final String       getFileName()        { return primaryFileName; }
   ...
   */
  public final Errors getInitErrors() { return initErrors; }

  /**
   * Returns Errors object containing errors found during parsing.
   */
  public final Errors getParseErrors() { return parseErrors; }

  /**
   * Returns Errors object containing errors found while parsing the
   * builtin operator and synonym tables.  Should never be interesting.
   */
  public final Errors getGlobalContextErrors() { return globalContextErrors; }

  /**
   * Returns Errors object containing errors found during semantic
   * processing and level checking.
   */
  public final Errors getSemanticErrors() { return semanticErrors; }
  ...
  public final void printParseUnitContext() {
    ...
    while (enumerate.hasMoreElements()) {
      ...
      ToolIO.out.println("  " + key + "-->" + 
                          ((ParseUnit)parseUnitContext.get(key)).getName());
    }
  }
  ...
  // neither happens.
  private final ParseUnit findOrCreateParsedUnit(
                            String name, Errors errors, boolean firstCall)
  ...
  private void nonCircularityTest(ParseUnit parseUnit, Errors errors ) 
  throws AbortException {
  ...
  private void nonCircularityBody(ParseUnit parseUnit, ParseUnit candidate, Errors errors, 
                                  HashSet alreadyVisited, Vector circularPath) 
                                  throws AbortException  {
    // If we have already checked for circularities through this
    // parseUnit, just return
    if (alreadyVisited.contains(candidate)) return;
    ...
    for ( int i = 0; i < referencees.size(); i++ ) {
      ...
      if ( referencee == parseUnit ) {
        ...
        errors.addAbort("Circular dependency among .tla files; " + 
                        "dependency cycle is:\n\n  " + 
        ...
        nonCircularityBody( parseUnit, referencee, errors, 
                            alreadyVisited, circularPath);
  ...
  private boolean findNextUnresolvedExtentionBody(ModulePointer currentModule, HashSet alreadyVisited) {
    ...
    for ( int i = 0; i < extendees.size(); i++ ) {
      ...
      if ( findNextUnresolvedExtentionBody(
                  currentContext.resolve((String)extendees.elementAt(i)),
                  alreadyVisited) ) {
        extentionFound = true;
        return true;
      }
    ...
    for ( int i = 0; i < instancees.size(); i++ ) {
      ...
      if ( currentContext.resolve((String)instancees.elementAt(i)) != null ) {
        ...
        if ( findNextUnresolvedExtentionBody( 
                currentContext.resolve((String)instancees.elementAt(i)),
                alreadyVisited ) ) {
          extentionFound = true;
          return true;
        } // end if
    ...
    for (int i = 0; i < innerModules.size(); i++) {
      if ( findNextUnresolvedExtentionBody(
              (ModulePointer)innerModules.elementAt(i), 
              alreadyVisited) ) {
        extentionFound = true;
        return true;
      }
  ...
  // A wrapper for the next method, which empties the alreadyVisited
  // table calling the recursive method below
  private boolean findNextUnresolvedInstantiation(ModulePointer 
                                                       currentModule) {
    HashSet alreadyVisited = new HashSet();
  ...
  private boolean instanceResolvesToInternalModule(
                     ModulePointer currentModule, String instanceeName) {
    ...
    for (int i = 0; i < body.heirs().length; i++) {
      ...
      // if we encounter an INSTANCE decl
        ...
        String instanceModuleName = 
                instanceHeirs[nonLocalInstanceNodeIX].heirs()[1].getImage();
  ...
  private boolean findNextUnresolvedInstantiationBody(
                    ModulePointer currentModule, HashSet alreadyVisited) {
    if (alreadyVisited.contains(currentModule)) {
    ...
    for (int i = 0; i < instancees.size(); i++) {
      ...
      if (currentContext.resolve((String)instancees.elementAt(i)) == null) {
        ...
        if (!instanceResolvesToInternalModule(
                 currentModule, (String)instancees.elementAt(i))) {
    ...
    for ( int i = 0; i < extendees.size(); i++ ) {
      // by recursive invocation of this method on the extendees
      if (findNextUnresolvedInstantiationBody(
              currentContext.resolve((String)extendees.elementAt(i)),
              alreadyVisited) ) {
        instantiationFound = true;
        return true;
    ...
    while ( additions ) {
      ...
      for (int i = lastAdditionsStart; i < lastAdditionsEnd; i++) {
        ...
        while ( enumModules.hasMoreElements() ) {
          ...
          if ( directlyExtends(modPointer, 
                               (ModulePointer)extenders.elementAt(i)) ) {
            if (!additions) lastAdditionsStart = lastAdditionsEnd; 
  ...
  private void resolveNamesBetweenSpecAndInstantiation(
                 ModulePointer instancer, ParseUnit instancee) {
    // Bind the name of the instancee in the instancer's context
    ModuleContext instancerContext = instancer.getRelatives().context;
    instancerContext.bindIfNotBound(instancee.getName(), 
                                    instancee.getRootModule());
  }
  ...
  // extendeeParseUnit
  private void resolveNamesBetweenSpecAndExtention(
                   ModulePointer extender, ParseUnit extendee) {
    // First, bind the name of the extendee in the extender's context
    ModuleContext   extenderContext   = extender.getRelatives().context;
    extenderContext.bindIfNotBound(extendee.getName(), 
                                   extendee.getRootModule());

    // Vextor of ModulePointers for modules that either are
    // "extender", or extend "extender" directly, of extend it
    // indirectly.
    Vector modulesIndirectlyExtending = 
                  getModulesIndirectlyExtending(extender);

    for (int i = 0; i < modulesIndirectlyExtending.size(); i++ ) {
      resolveNamesBetweenModuleAndExtention(
         (ModulePointer)modulesIndirectlyExtending.elementAt(i), extendee);
    }
  }
  ...
  private void resolveNamesBetweenModuleAndExtention(
                  ModulePointer extenderModule, ParseUnit extendeeParseUnit) {
    ...
    for (int i = 0; i < extendedNames.size(); i++) {
      ...
      Vector extendeeInnerModules = 
               extendeeParseUnit.getRootModule().getDirectInnerModules();
      ...
      for (int j = 0; j < extendeeInnerModules.size(); j++) {
        ModulePointer extendeeInnerModule = 
                       ((ModulePointer)extendeeInnerModules.elementAt(j));
    ...
    for (int i = 0; i < extenderInnerModules.size(); i++) {
      ModulePointer nextInner = 
                      (ModulePointer)extenderInnerModules.elementAt(i);
   ...
   */
  public boolean loadSpec(String rootExternalModuleName, Errors errors)
  throws AbortException { 
    ...
    rootParseUnit = findOrCreateParsedUnit(
                       rootExternalModuleName, errors, true /* first call */); 
    ...
    while (findNextUnresolvedExtention(rootModule) ||
      ...
      if ( parseUnitContext.get( nextParseUnitName ) == null ) {
        ...
        nextExtentionOrInstantiationParseUnit = 
          findOrCreateParsedUnit( 
             nextParseUnitName, errors, false /* not first call */ );
      }
      else {
        // or find it in the known parseUnitContext
        nextExtentionOrInstantiationParseUnit = 
          (ParseUnit)parseUnitContext.get( nextParseUnitName );
      }
      ...
      ParseUnit extenderOrInstancerParseUnit = 
                  nextExtenderOrInstancerModule.getParseUnit();

      if ( extentionFound ) {
        extenderOrInstancerParseUnit.addExtendee(  
                nextExtentionOrInstantiationParseUnit  );
        nextExtentionOrInstantiationParseUnit.addExtendedBy(  
                extenderOrInstancerParseUnit  );
      }
      if ( instantiationFound ) {
        extenderOrInstancerParseUnit.addInstancee(  
                nextExtentionOrInstantiationParseUnit  );
        nextExtentionOrInstantiationParseUnit.addInstancedBy(  
                extenderOrInstancerParseUnit  );
      }
      ...
      if (extentionFound) {
        resolveNamesBetweenSpecAndExtention(
            nextExtenderOrInstancerModule, 
            nextExtentionOrInstantiationParseUnit);
      }
      ...
      if ( instantiationFound ) {
        resolveNamesBetweenSpecAndInstantiation(
              nextExtenderOrInstancerModule, 
              nextExtentionOrInstantiationParseUnit);
      }
    } // end while
