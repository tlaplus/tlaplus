...
public class ProverHelper
{
    ...
    public static void runProverForActiveSelection(boolean checkStatus, boolean checkProofs)
    {
        ...
        /*
         * The following work is now done within ProverJob. This
         * was done for technical reasons. In particular, the following
         * code could initiate a parsing of the module. This parsing could
         * be redundant if the user also saves the module and has auto parse
         * on save activated. In that case, the two parses happen quickly back
         * to back. SANY has a bunch of static fields that get initialized
         * at the beginning, so running it concurrently can cause some
         * problems. Now, the second potential parsing is done in the prover job's
         * run method. The run method for the job is not executed until background autobuilding
         * (parsing) is finished, so the second parse will not occur until after the first is complete.
         */
        // /*
        ...
        // ProverJob proverJob = new ProverJob(checkStatus, parseResult.getSpecObj()
        ...
        // * Step 7 *
        // ***********************************************************/
        // ProverJob proverJob = new ProverJob(checkStatus, nodeToProve, checkProofs);
        // proverJob.setUser(true);
        // proverJob.setRule(new ProverJobRule());
        // proverJob.schedule();
    }
