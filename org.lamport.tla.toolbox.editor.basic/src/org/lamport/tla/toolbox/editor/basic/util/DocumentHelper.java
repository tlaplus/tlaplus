...
public class DocumentHelper
{
    ...
    /**
     * Returns the line number (in Java coordinates) of the line containing the first
        ...
        try {
            if (fairStringLocation == -1 || 
               ((algorithmStringLocation != -1) && (algorithmStringLocation < fairStringLocation))) {
                return LocationToLine(document, algorithmStringLocation);
            }
            int i = fairStringLocation + 6 ;
            while (Character.isWhitespace(moduleAsString.charAt(i))) {
                i++ ;
            } ;
            if ((i != fairStringLocation + 6) && 
                (moduleAsString.startsWith("algorithm", i)) &&
                !Character.isLetterOrDigit(moduleAsString.charAt(i+9))) {
                return LocationToLine(document, fairStringLocation);
            } 
            else {
                return LocationToLine(document, algorithmStringLocation);
            }
