...
public class BoxedCommentHandler extends AbstractHandler implements IHandler
{
    ...
    int endCommentOffset; // the offset immediately FOLLOWING '*)' ending the comment.
     ...
     *  - Any non-white-space text following the (*..* on the same line is put on a new
     *    line with no beginning white space.
     *  - If there is any non-white-space text preceding the *...*), then the
     *    *...*) is moved to a new line.
     *  - If there is any non-white-space following the *...*), then a new
     *    line is begun immediately after the *...*) token.
     ...
     *  - The *...*) token is replaced by a (*....*) token the same width as
     *    and starting at the same column as the opening one.
     ...
     *    not to the left of the *) of the starting (*...*) token.
     * Returns false iff it detects an error.
     */
    private boolean boxComment() throws org.eclipse.jface.text.BadLocationException
    {
        // set begin/endCommentOffset

        int beginCommentLength; // the offset of the '(' of the '(*' starting the comment.
        ...
        // int firstLineLength = 0; // then these are its length & offset, else offset = -1
        // int lastLineOffset = -1; // if there's an extra line after the '(*...*'
        // int lastLineLength = 0; // then these are its length & offset, else offset = -1
        ...
        // endCommentLineInfo = doc.getLineInformationOfOffset(endCommentOffset);
        endCommentLength = 2;
        ...
        {
            String singleLine = doc.get(beginCommentOffset + beginCommentLength, endCommentOffset
                    - (beginCommentOffset + beginCommentLength) - endCommentLength);
            buffer.append(makeCommentBoxLine(StringHelper.trimFront(singleLine)));
            buffer.append(StringHelper.newline);
        } else
        {

            // Produce a first line for anything after the (* on its line.
            String restOfLine = doc.get(beginCommentOffset + beginCommentLength, doc.getLineOffset(beginLine)
                    + doc.getLineLength(beginLine) - (beginCommentOffset + beginCommentLength));
            if (!StringHelper.onlySpaces(restOfLine))
            {
                buffer.append(makeCommentBoxLine(StringHelper.trimFront(restOfLine)));
                buffer.append(StringHelper.newline);
            }

            for (int i = beginLine + 1; i < endLine; i++)
            {
                buffer.append(makeCommentBoxLine(doc.get(doc.getLineOffset(i), doc.getLineLength(i))));
                buffer.append(StringHelper.newline);
            }

            // Produce a last line for anything before the *) on its line.
            int lineOffset = doc.getLineOffset(endLine);
            String startOfLine = doc.get(lineOffset, endCommentOffset - endCommentLength - lineOffset);
        ...
        if (!StringHelper.onlySpaces(doc.get(endCommentOffset, doc.getLineOffset(endLine) - endCommentOffset
                + doc.getLineLength(endLine))))
        {
            buffer.append(StringHelper.newline);
        }

        doc.replace(beginCommentOffset, endCommentOffset - beginCommentOffset, buffer.toString());

        return true;
    }

    /*
     * Removes the box from a comment in the obvious way so
     * that boxing it should produce the same result.  However,
     * anything outside the box but on the same line as the
     * box will be deleted, except for text preceding the
     ...
     * ending *).  A line beginning with a space character is left unchanged.
     * A sequence of lines beginning with non-space characters are considered
     * to be a single paragraph and are formatted as such for boxing.
     * 
     * Returns false iff it detects an error.
     */
    private final boolean formatComment() throws org.eclipse.jface.text.BadLocationException
    {
        setCommentFields();
        ...
        for (int i = beginCommentLine + 1; i < endCommentLine; i++)
        {
            ...
            if ((currentLine.length() == 0) || (Character.isSpaceChar(currentLine.charAt(0))))
            {
                ...
                // Output a StringHelper.newline if there's an unfinished paragraph.
        ...
        doc.replace(beginReplacementOffset, endReplacementOffset - beginReplacementOffset, buffer.toString());
     ...
     */
    private void setCommentFields() throws org.eclipse.jface.text.BadLocationException
    {
        beginCommentOffset = text.lastIndexOf("(*", offset);
        endCommentOffset = text.indexOf("*)", offset) + 2;
    ...
    {
        return StringHelper.copyString(" ", indent) + "(*" + StringHelper.copyString("*", margin - indent - 4) + "*)";
    }

    /*
     * Returns the box's starting (******) line, with no spaces at the
     * beginning and no newline at end.
     */
    ...
    private final String makeCommentBoxLine(String str)
    {
        ...
        return StringHelper.copyString(" ", indent) + "(* " + trimmed + StringHelper.copyString(" ", endSpaces) + " *)";
    }
